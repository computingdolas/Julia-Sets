...
std::string ker_code = "\n"\
"__kernel void juliaImage(__global unsigned char *color_bit_device, __global unsigned int *N, __global double *mesh, __global double *threshold)\n"\
"{																				   \n"\
" 	 unsigned int idx = get_global_id(0);											   \n"\
"    double real = (mesh[0] * (idx % N[0])) - 2.0 ;                                   \n"\
"    double imag = (mesh[0] * (idx / N[0])) - 2.0 ;                                   \n"\
...
int main(int argc,char *argv[]){
    ...
    cl::Buffer mesh_device(context,CL_MEM_READ_WRITE,sizeof(double));
    cl::Buffer threshold_device(context,CL_MEM_READ_WRITE,sizeof(double));

    //Create command queue
    cl::CommandQueue queue(context,def_device);

    //Copying data to the buffers
    queue.enqueueWriteBuffer(mesh_device,CL_TRUE,0,sizeof(double),&mesh);
    queue.enqueueWriteBuffer(numPixels1D_device,CL_TRUE,0,sizeof(unsigned int),&numPixels1D);
    queue.enqueueWriteBuffer(threshold_device,CL_TRUE,0,sizeof(double),&threshold);
